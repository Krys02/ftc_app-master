...
public abstract class OpenCVPipeline implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    //OpenCV logic
    private OpenCV.CameraMode cameraMode = OpenCV.CameraMode.BACK;
    private boolean isVuforia = false;
    private boolean isVuMark = false;
    private volatile boolean isOpenCVEnabled = true;
    ...
    private volatile CVVuforia cvvuforia = null;
    ...
    private volatile OpenCV.VuMark vuMarkFound = OpenCV.VuMark.NONE;
    ...
    public void init(Context context, ViewDisplay viewDisplay) {
        init(context, viewDisplay, OpenCV.CameraMode.BACK, false, null);
    }
     ...
     * @param viewDisplay the ViewDisplay that will display the underlying JavaCameraView to the screen;
     *                    in most cases, using CameraViewDisplay.getInstance() as the argument is just fine.
     * @param cameraMode Which camera is to be used, will be a OpenCV.CameraMode
     * @param findVuMarks A boolean. True to scan for vumarks, false to ignore them
     */
    public void init(Context context, ViewDisplay viewDisplay, OpenCV.CameraMode cameraMode, boolean findVuMarks) {
     ...
     */
    public void init(Context context, ViewDisplay viewDisplay, OpenCV.CameraMode cameraMode, boolean findVuMarks, CameraName webcamName) {
        ...
        if(isVuforia && this.VUFORIA_KEY == null) { //Checks that a license is provided.
            ...
            //Instantiates CVVuforia, and gives it access to this detector's analyzeFrame method
            cvvuforia = new CVVuforia(parameters) {
                @Override
                public Mat analyzeFrame(Mat rgba, Mat gray) {
                    return processFrame(rgba, gray);
                }
            };
            cvvuforia.enableConvertFrameToBitmap();
    ...
    public void enable() {
        ...
        //Runs enabling sequence for CVVuforia
        if(isVuforia){
            final Activity activity = (Activity) context;
            final int cameraMoniterViewID = context.getResources().getIdentifier("cameraMonitorViewId", "id", context.getPackageName());
            //Updates displayed view
            viewDisplay.setCurrentView(context, rawView);
            //Gives cvvuforia access to the correct View
            cvvuforia.setRawView(rawView);
            //Starts cvvuforia
            cvvuforia.start();
    ...
    public synchronized void enableVuMarkDetection() {
        if(cvvuforia != null) cvvuforia.enableTrack();
    ...
    public synchronized void disableVuMarkDetection() {
        if(cvvuforia != null) cvvuforia.disableTrack();
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        if(isOpenCVEnabled) return processFrame(rgba, gray);
        else return rgba;
    }
    ...
    private void loadVuMarks() {
        targetsRoverRuckus = this.cvvuforia.loadTrackablesFromAsset("RoverRuckus");
    ...
    /**
     * Returns a OpenCV.Vumark enum representing which VuMark was found.
     * Will only re-check every 5ms, so don't worry about calling it repeatedly
     * @return The VuMark found, will return OpenCV.VuMark.NONE if no VuMark is found.
     */
    public synchronized OpenCV.VuMark findVuMark() {
     ...
     */
    private OpenCV.VuMark convert2VuMark(String name) {
        switch (name) {
            case "Blue-Rover":
                return OpenCV.VuMark.BLUE_ROVER;
            case "Red-Footprint":
                return OpenCV.VuMark.RED_FOOTPRINT;
            case "Front-Craters":
                return OpenCV.VuMark.FRONT_CRATERS;
            case "Back-Space":
                return OpenCV.VuMark.BACK_SPACE;
            default:
                return OpenCV.VuMark.NONE;
        }
    }
